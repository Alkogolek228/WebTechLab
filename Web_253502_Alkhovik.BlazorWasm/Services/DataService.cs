...
public class DataService : IDataService
{
	...
	public async Task GetProductListAsync(string? categoryNormalizedName, int pageNo = 1)
	{
		...
		try
		{
			var route = new StringBuilder($"{_httpClient.BaseAddress.AbsoluteUri}car/");
			...
			var data = await response.Content.ReadFromJsonAsync<ResponseData<ProductListModel<Car>>>
	...
	public async Task GetCategoryListAsync(int pageNo = 1)
	{
		...
		var urlString = $"{_httpClient.BaseAddress.AbsoluteUri}category/";
		try
		{
